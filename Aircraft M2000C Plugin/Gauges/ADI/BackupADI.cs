...
namespace GadrocsWorkshop.Helios.Gauges.M2000C.ADI.Backup
{
    ...
    [HeliosControl("Helios.M2000C.ADI.Backup", "Backup ADI", "M-2000C", typeof(BackgroundImageRenderer), HeliosControlFlags.NotShownInUI)]
    class BackupADI : CompositeVisualWithBackgroundImage
    {
        ...
        public BackupADI()
            : base("Backup ADI", new Size(400, 400))
        {
            SupportedInterfaces = new[] { typeof(Interfaces.DCS.M2000C.M2000CInterface) };
            AddGauge("Backup ADI", new Point(0d, 0d), new Size(400d, 400d), _interfaceDeviceName, "Backup ADI");
            _frameGlassPanel = AddPanel("Gauge Glass", new Point(0d, 0d), new Size(400d, 400d), REFLECTION_IMAGE, _interfaceDeviceName);
        ...
        {
            BackupADIGauge _display = new BackupADIGauge(name, new Size(400, 400), _interfaceDeviceName)
            {
                Top = pos.Y,
                Left = pos.X,
                Height = size.Height,
