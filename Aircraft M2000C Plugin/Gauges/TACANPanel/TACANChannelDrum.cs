...
namespace GadrocsWorkshop.Helios.Gauges.M2000C.TACAN
{
    ...
    public class TACANChannelDrum : BaseGauge
    {
        ...
        public TACANChannelDrum(string name, Point posn, Size size)
            : base(name, size)
        {
            _xModeImage = new GaugeImage("{M2000C}/Gauges/TACANPanel/tacan_channel_x_mode.xaml", new Rect(posn.X, posn.Y, size.Width, size.Height));
            Components.Add(_xModeImage);

            _yModeImage = new GaugeImage("{M2000C}/Gauges/TACANPanel/tacan_channel_y_mode.xaml", new Rect(posn.X, posn.Y, size.Width, size.Height));
